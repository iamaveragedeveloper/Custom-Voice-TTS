...
def test_pipeline(tmp_path):
    ...
    lines = []
    for i in range(8):
        save_wav(raw / f"u{i}.wav", fake_voice(1.5 + 0.2 * i, 100 + 5 * i), SR)
        lines.append(f"u{i}|{WORDS[i % len(WORDS)]}")
    (tmp_path / "meta.csv").write_text("\n".join(lines), encoding="utf-8")
    data = tmp_path / "data"
    prep(raw, data, metadata=tmp_path / "meta.csv")
    run = tmp_path / "run"
    train_acoustic(data, run, steps=6, max_frames=3000, log_every=2, save_every=3)
